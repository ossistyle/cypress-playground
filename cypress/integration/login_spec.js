...
describe('/login.html', () => {
  ...
  it('greets with welcome message', () => {
    cy.get('h1').should('have.text', i18n['login_welcome_message'][language]);
  });

  it('requires username', () => {
    cy.get('#username').type('dfsadfas{enter}');
    cy.get('#error').should('contain', i18n['login_values_not_match'][language]);
  });
  ...
  it('has a placeholder for username', () => {
    cy.get('#username').should('have.attr', 'placeholder', i18n['login_username_placeholder'][language]);
  });

  it('has a placeholder for username', () => {
    cy.get('#password').should('have.attr', 'placeholder', i18n['login_password_placeholder'][language]);
  });
});
